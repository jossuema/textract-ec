#!/usr/bin/env python3
"""Lab 1 (checkpoint resuelto) — texto y confianza con DetectDocumentText (US$ 0.0015/página aprox.).

Versión completa del Lab 1 con un extra: la lista de las 5 líneas de MENOR confianza (para
recuperarte si te atrasas: cp checkpoints/lab1/01_texto.py . y ejecuta igual).
...
def imprimir_peores_lineas(lineas: list[tuple[str, float]], n: int = 5) -> None:
    """Extra del checkpoint: las n líneas de menor confianza (lo primero que revisaría un humano)."""
    consola.titulo(f'Las {n} líneas de menor confianza')
    peores = sorted(enumerate(lineas, 1), key=lambda par: par[1][1])[:n]
    for i, (txt, conf) in peores:
        print(f'{i:>3}  {consola.confianza_coloreada(conf)}  {txt}')
    if peores and peores[0][1][1] >= UMBRAL:
        consola.ok('incluso la peor línea supera 90: documento limpio')
    else:
        print('en la foto suelen caer aquí montos, la clave de acceso y el pie de página: justo lo que más importa.')
...
def main(argv: list[str] | None = None) -> int:
    ...
    lineas = imprimir_lineas(resp)
    imprimir_peores_lineas(lineas)
